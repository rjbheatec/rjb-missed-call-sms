...
if __name__ == '__main__':
        app.run(host='0.0.0.0', port=int(os.environ.get('PORT', 5000)))def sms_missed_new(n):
...
def get_sms(n):
        return (
                    'Hi, thanks for calling ' + n['brand'] + '! We are on hand to help. '
                    'Message us anytime on WhatsApp wa.me/' + WHATSAPP_NUMBER +
                    ' or visit ' + n['website'] + ' to book online. '
                    'Alternatively call us back on ' + n['display']
        )
...
if __name__ == '__main__':
        app.run(host='0.0.0.0', port=int(os.environ.get('PORT', 5000)))return (f"Thanks for calling {n['brand']}! Great speaking with you. "
